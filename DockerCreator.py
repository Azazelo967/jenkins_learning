...
class DockerCreator:
    ...
    def run_jenkins_agent(self):
        with open("/home/al/jenkins_learning/id_rsa.pub", "r") as key_file:
            ssh_key = key_file.readline()
        subprocess.run("echo ssh key is " + ssh_key, shell=True)
        return subprocess.run("docker run -d --rm --name jenkins_agent -p 4444:22 -e 'JENKINS_AGENT_SSH_PUBKEY=" + ssh_key.rstrip() + "' " + "jenkins_agent:2", shell=True, capture_output=True)
